...
export default function FormInvoiceHistories({
  open,
  onOpenChange,
  selectRow,
}: {
  ...
  selectRow: INegotiationInvoice | null;
}) {
  const form = useForm<INegotiationInvoice>({
    mode: "all",
  });

  const { pending: formPending } = useFormStatus();
  const [pending, setPending] = useState<boolean>(false);
  const [history, setHistory] = useState<IInvoiceHistory[]>([]);

  const onSubmit: SubmitHandler<INegotiationInvoice> = async (data) => {
    console.log(data);
  ...
  useEffect(() => {
    if (selectRow && selectRow.id) {
      const refreshHistory = async () => {
        setPending(true);
        fetchInvoiceHistories(selectRow.id).then((data) => {
          setHistory(data);
        });
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="p-2">
                  <FormLabel>
                    ...
                    <Input
                      type="date"
                      required
                      value={formatDateForInput(selectRow.datPrest)}
                      disabled
                      className="mb-1"
                    />
                  </FormLabel>
