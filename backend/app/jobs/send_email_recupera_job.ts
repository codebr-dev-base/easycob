...
interface SendEmailRecuperaJobPayload {
  action_id: number;
  codigo: string;
  credor: string;
  regis: string;
...
export default class SendEmailRecuperaJob extends Job {
   ...
   * Base Entry point
   */
  async handle(payload: SendEmailRecuperaJobPayload) {
   ...
   */
  async rescue(payload: SendEmailRecuperaJobPayload) {
    const actionService = new ActionService();
