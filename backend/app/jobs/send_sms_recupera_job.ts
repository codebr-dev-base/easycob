...
interface SendSmsRecuperaJobPayload {
  action_id: number;
  codigo: string;
  credor: string;
  regis: string;
...
export default class SendSmsRecuperaJob extends Job {
   ...
   * Base Entry point
   */
  async handle(payload: SendSmsRecuperaJobPayload) {
   ...
   */
  async rescue(payload: SendSmsRecuperaJobPayload) {
    const actionService = new ActionService();
